...
def tournament_info(message, identify, typ):  # typ = 1 - команда вызвана админом, = 0 - обычным пользователем,
    ...
    if typ == 1:
        ...
        markup.add(types.InlineKeyboardButton('🙍‍♂️Посмотреть участников', callback_data=identify + 'members'))
    else:
        markup.add(types.InlineKeyboardButton('📌Вернуться в меню', callback_data='menu'))
        if status == 1:
...
def mero_members(message, identify, typ, club=None):  # typ = 0 - вызвана при ручном изменении участников турнира
    status, type_tour = 0, ''
    if club:
    ...
    markup = types.InlineKeyboardMarkup()
    if club:
        markup.add(types.InlineKeyboardButton('🙍‍♂️Обновить', callback_data=club + identify + 'members'))
    else:
        markup.add(types.InlineKeyboardButton('🙍‍♂️Обновить', callback_data=identify + 'members'))
    if typ == 1:
        for i in players:
            if i[1] == str(message.chat.id):
                if club:
...
def accept_request(message, identify, member_id, club=None):
    if club:
        ...
        base_tour = sqlite3.connect('clubs')
        cur_tour = base_tour.cursor()
        cur_tour.execute(f'SELECT name FROM {club} WHERE identify = ?', (identify,))
    else:
        base = sqlite3.connect('tournament')
        cur = base.cursor()
        cur.execute(f'UPDATE {identify} SET request = ? WHERE id = ?', (1, member_id))
        base_tour = sqlite3.connect('tournaments_list')
        cur_tour = base_tour.cursor()
        cur_tour.execute(f'SELECT name FROM tournaments WHERE identify = ?', (identify,))
    base.commit()
    cur.close()
    base.close()
    name_base = cur_tour.fetchall()
    name = ''
    for i in name_base:
        name = i[0]
        break
    bot.send_message(member_id, f'✅Ваша заявка на участие в {name} принята!')
...
def decline_request(message, identify, member_id, club=None):
    ...
    for i in name_base:
        name = i[0]
        break
    bot.send_message(member_id, f'❌Ваша заявка на участие в {name} отклонена')
...
def us_help(message):
    output = """
    Привет! Это краткий гайд как пользоваться ботом.

Чтобы записаться на вечер в клубе:
1. /menu
2. Игры в клубах
3. Выбери клуб
...
Чтобы изменить свой профиль или добавить свою фотографию:
1. /menu
2. Профиль
3. Изменить профиль
...
def ad_help(message):
    output = """
    Привет! Это краткий админ гайд как пользоваться ботом.

Ты можешь создавать и изменять мероприятия в своем клубе. Можно создавать закрытые (ты должен сам забить участников), открытые (каждый сможет зарегистрироваться) и мероприятия по заявке.

Чтобы создать мероприятие:
1. /admin
2. Выбери свой клуб
3. Нажми "Создать мероприятие"
4. Заполни форму
!!!МЕРОПРИЯТИЕ СОЗДАЕТСЯ ЗАКРЫТЫМ, то есть его никто не видит и не может в него записаться. Чтобы каждый смог записаться, надо изменить статус мероприятия.
Пожалуйста, удаляй мероприятия после того, как они прошли (можно спустя какое-то время)
...
Ещё ты можешь рассылать какие либо сообщения участникам конкретного мероприятия и брать картинки профилей для анонсов.
Чтобы это сделать:
1. /admin
2. Выбери свой клуб
...
Эти сообщения придут только участникам мероприятия (тем, у кого одобрена заявка, если мероприятие по заявке). Подтверждение участия - это просто напоминание участникам о мероприятии. Добавляет им галочку в списке участников

Если ты хочешь создать ПЛАТНОЕ мероприятие, то можно сделать сбор денег напрямую через бота, но необходимо заранее предупредить об этом @alexstrazza И @Glasarss
"""
    bot.send_message(message.chat.id, output)
...
— <a href="https://t.me/mafia_itmo">Чёрный Ход</a>, ИТМО
18:30-22:00
🏛️Кронверский 49
...
— <a href="https://vk.com/club219471408">LETI-MAFIA</a>, университет ЛЭТИ
17:30-22:30
...
— <a href="https://vk.com/club154981253">BONCHMAFIA</a>, Бонч-Бруевича
15:00-21:00
